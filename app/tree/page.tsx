...
function BranchFocusView({ branch, unlocked, onNodeClick, onBranchClick, selectedBranch }: {
  ...
  // Branch auto-lights when ALL cores are unlocked
  const allCoresUnlocked = cores.every(c => unlocked.has(c.id))
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} className="block">
      ...
      {/* Branch → core lines — light up per core */}
      {cPositions.map((corex, i) => {
        const coreLit = unlocked.has(cores[i].id)
        return <path key={`bc-${i}`} d={buildPath(cx, branchY - 20, corex, coreY + 13)} fill="none" stroke={col} strokeWidth={coreLit ? 2 : 1} strokeOpacity={coreLit ? 1 : 0.3} strokeDasharray={coreLit ? undefined : "4 3"} />
      })}
      ...
      {/* Branch node — auto-lights when all cores unlocked, always opens tricks panel */}
      <g transform={`translate(${cx}, ${branchY})`} onClick={() => onBranchClick(branch)} style={{ cursor: "pointer" }}>
        ...
        {allCoresUnlocked && <circle r={28} fill="none" stroke={col} strokeWidth="1.5" opacity={0.35} strokeDasharray="4 2" />}
        <circle r={22} fill={col} fillOpacity={allCoresUnlocked ? 1 : 0.35} stroke={col} strokeWidth={allCoresUnlocked ? 2 : 1.5} />
        <text y="1" textAnchor="middle" fontSize="10" fill="white" dominantBaseline="middle" fontWeight="bold">
          {branchLabel[branch]}
        </text>
        {allCoresUnlocked && <text y={13} textAnchor="middle" fontSize="7" fill="white" opacity={0.8}>✓</text>}
      </g>

      {/* Core nodes — clickable, light up when unlocked */}
      {cores.map((core, i) => {
        const coreLit = unlocked.has(core.id)
        return (
          <g key={core.id} transform={`translate(${cPositions[i]}, ${coreY})`}
            onClick={() => onNodeClick(core.id)} style={{ cursor: "pointer" }}>
            <circle r={13} fill={col} fillOpacity={coreLit ? 1 : 0.2} stroke={col} strokeWidth={coreLit ? 2 : 1.5} />
            {coreLit && <text y="1" textAnchor="middle" fontSize="8" fill="white" dominantBaseline="middle">✓</text>}
            <text y={-20} textAnchor="middle" fontSize="9" fill={coreLit ? "#e4e4e7" : "#a1a1aa"}>{core.label}</text>
  ...
  return (
    <svg width={FW} height={FH} viewBox={`0 0 ${FW} ${FH}`} className="block">
      ...
      {FULL_NODES.map(n => {
        ...
        const { x, y } = pos
        const branchCores = n.branch ? (BRANCH_CORES[n.branch as Exclude<Branch, "foundation">] ?? []) : []
        const allCoresLit = n.branch && branchCores.length > 0 && branchCores.every(c => unlocked.has(c.id))
        ...
        const isSelected = n.branch && selectedBranch === n.branch
        const coreNode = n.type === "core"
        const coreLit = coreNode && unlocked.has(n.id)
        return (
          <g key={n.id} transform={`translate(${x},${y})`} onClick={() => onNodeClick(n)} style={{ cursor: "pointer" }}>
            ...
            {allCoresLit && <circle r={r + 7} fill="none" stroke={n.branch ? branchColor[n.branch] : "#fff"} strokeWidth="1.5" opacity={0.35} strokeDasharray="4 2" />}
            <circle r={r} fill={col} fillOpacity={n.branch ? (allCoresLit ? 1 : isSelected ? 1 : 0.4) : (unlocked.has(n.id) ? 1 : 0.18)} stroke={col} strokeWidth={allCoresLit ? 2 : 1.5} />
            {(unlocked.has(n.id) && !n.branch) || coreLit ? <text y="1" textAnchor="middle" fontSize="8" fill="white" dominantBaseline="middle">✓</text> : null}
            <text y={-(r + 7)} textAnchor="middle" fontSize="9" fill={coreLit ? "#e4e4e7" : "#a1a1aa"}>{n.label}</text>
...
export default function TrickTree() {
  ...
  function handleFullNodeClick(node: TrickNode) {
    if (node.branch) setSelectedBranch(prev => prev === node.branch ? null : node.branch!)
      ...
      // toggle core or foundation node
      setUnlocked(prev => { const n = new Set(prev); n.has(node.id) ? n.delete(node.id) : n.add(node.id); return n })
    }
  }
