...
const BRANCH_CORES: Record<
  Exclude<Branch, "foundation">,
  { id: string; label: string; sublabel?: string }[]
> = {
  draaien: [
    { id: "rotatie", label: "Rotatie" },
    { id: "contrarotatie", label: "Contra-rotatie" },
  ],
  balans: [
    { id: "voor-achter", label: "Voor-Achter" },
    { id: "tip-hielen", label: "Tip-Hielen" },
    { id: "hoog-laag", label: "Hoog-Laag" },
  ],
  springen: [
    { id: "sprong", label: "Sprong" },
...
const FULL_NODES: TrickNode[] = [
  ...
  },
  {
    ...
    y: 220,
    type: "branch",
    prereqs: ["stilstaand", "rijdend", "rolhouding", "durven"],
    branch: "draaien",
  },
  {
    id: "balans",
    label: "Balans /",
    sublabel: "Zwaartepunt",
    x: 165 + PAD,
    y: 220,
    type: "branch",
    prereqs: ["stilstaand", "rijdend", "rolhouding", "durven"],
    ...
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} className="block">
      ...
      {fPositions.map((fx, i) => {
        ...
        return (
          <path
            key={`ft-${i}`}
            d={buildPath(fx, foundY - 20, cx, trunkY)}
            fill="none"
            stroke={lit ? "#BA7517" : "#3f3f46"}
            strokeWidth={lit ? 2 : 1}
      ...
      {cPositions.map((corex, i) => (
        <path
          key={`bc-${i}`}
          d={buildPath(cx, branchY - 20, corex, coreY + 13)}
          fill="none"
          stroke={col}
      ...
      {FOUNDATIONS.map((f, i) => {
        ...
        return (
          <g
            key={f.id}
            transform={`translate(${fPositions[i]}, ${foundY})`}
            onClick={() => onNodeClick(f.id)}
            style={{ cursor: "pointer" }}
          >
            <circle
              r={18}
              fill={lit ? "#BA7517" : "#52525b"}
              fillOpacity={lit ? 1 : 0.2}
              stroke={lit ? "#BA7517" : "#52525b"}
              strokeWidth={1.5}
            />
            {lit && (
              <text
            ...
            <text y={-26} textAnchor="middle" fontSize="8" fill="#a1a1aa">
              {f.label}
            </text>
            {f.sublabel && (
              <text y={-16} textAnchor="middle" fontSize="8" fill="#a1a1aa">
                {f.sublabel}
              </text>
            )}
          </g>
      ...
      {/* Branch node */}
      <g
        transform={`translate(${cx}, ${branchY})`}
      ...
      {cores.map((core, i) => (
        <g
          key={core.id}
          transform={`translate(${cPositions[i]}, ${coreY})`}
          onClick={() => onNodeClick(core.id)}
          style={{ cursor: "pointer" }}
        >
          <circle
            r={13}
            ...
            strokeWidth={1.5}
          />
          <text y={-20} textAnchor="middle" fontSize="9" fill="#a1a1aa">
            {core.label}
          </text>
          {core.sublabel && (
            <text y={-10} textAnchor="middle" fontSize="9" fill="#a1a1aa">
              {core.sublabel}
            </text>
          )}
        </g>
    ...
    <svg width={FW} height={FH} viewBox={`0 0 ${FW} ${FH}`} className="block">
      {foundations.map((n) => {
        ...
        return (
          <path
            key={`ft-${n.id}`}
            d={buildPath(p.x, p.y - typeRadius.foundation, TRUNK_X, TRUNK_Y)}
            fill="none"
            stroke={lit ? "#BA7517" : "#3f3f46"}
            strokeWidth={lit ? 2 : 1}
            strokeDasharray={lit ? undefined : "4 3"}
      ...
      {branches.map((n) => {
        ...
        return (
          <path
            key={`tb-${n.id}`}
            d={buildPath(TRUNK_X, TRUNK_Y, c.x, c.y + typeRadius.branch)}
            fill="none"
            stroke={col}
      ...
      {cores.map((n) => {
        const c = posMap[n.id];
        const p = posMap[n.prereqs[0]];
        if (!p || !c) return null;
        const parentNode = FULL_NODES.find((nd) => nd.id === n.prereqs[0]);
        const col = parentNode?.branch
          ? branchColor[parentNode.branch]
          : "#3f3f46";
        const isSelected =
          parentNode?.branch && selectedBranch === parentNode.branch;
        return (
          <path
            key={`bc-${n.id}`}
            d={buildPath(
              p.x,
              p.y - typeRadius.branch,
              c.x,
              c.y + typeRadius.core,
            )}
            fill="none"
      ...
      {FULL_NODES.map((n) => {
        ...
        const { x, y } = pos;
        const col = n.branch
          ? selectedBranch === n.branch
            ? branchColor[n.branch]
            : branchColor[n.branch] + "80"
          : unlocked.has(n.id)
            ? "#BA7517"
            : "#52525b";
          ...
          >
            {isSelected && (
              <circle
                r={r + 6}
                fill="none"
                stroke={n.branch ? branchColor[n.branch] : "#fff"}
                strokeWidth="2"
                opacity={0.5}
              />
            )}
            ...
            <text y={-(r + 7)} textAnchor="middle" fontSize="9" fill="#a1a1aa">
              {n.label}
            </text>
            {n.sublabel && (
              <text
...
export default function TrickTree() {
  ...
  function handleFullNodeClick(node: TrickNode) {
    if (node.branch)
      setSelectedBranch((prev) => (prev === node.branch ? null : node.branch!));
    else
      setUnlocked((prev) => {
  ...
  const onWheel = useCallback((e: React.WheelEvent) => {
    e.preventDefault();
    setScale((prev) =>
      Math.min(Math.max(prev * (e.deltaY > 0 ? 0.92 : 1.08), 0.35), 3),
    );
  }, []);
  ...
  const selectedTricks = selectedBranch
    ? getTricksByBranch(selectedBranch)
    : [];
  const tricksByLevel = selectedTricks.reduce(
    (acc, t) => {
      ...
      <div className="max-w-6xl mx-auto px-3 py-4 space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {mode === "full" && (
            <button
              onClick={resetView}
              className="px-3 py-1.5 rounded-lg text-xs bg-zinc-800 text-zinc-400 hover:text-white"
            >
              Reset
            </button>
          )}
            ...
            {BRANCHES.map((b) => (
              <button
                ...
                style={
                  activeBranch === b
                    ? {
                        background: branchColor[b],
                        borderColor: branchColor[b],
                        color: "#000",
                      }
                    : {
                        borderColor: branchColor[b] + "60",
                        color: branchColor[b],
                      }
                }
              >
            ...
            {mode === "branch" ? (
              <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-4">
                <BranchFocusView
                  ...
                  onBranchClick={(b) =>
                    setSelectedBranch((prev) => (prev === b ? null : b))
                  }
              ...
              >
                ...
                >
                  <FullTreeSVG
                    selectedBranch={selectedBranch}
                    unlocked={unlocked}
                    onNodeClick={handleFullNodeClick}
                  />
                </div>
            ...
            <div className="w-64 flex-shrink-0">
              <div className="bg-zinc-900 rounded-2xl border border-zinc-800 overflow-hidden sticky top-4">
                <div
                  ...
                  style={{
                    borderLeftWidth: 3,
                    borderLeftColor: branchColor[selectedBranch],
                  }}
                >
                  <div>
                    ...
                    return (
                      <div key={lvl}>
                        ...
                        {tricks.map((trick) => (
                          <div
                            key={trick.id}
                            className="px-4 py-2 border-b border-zinc-800/40 flex items-center gap-2"
                          >
                            <div
                              className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${trick.certified ? "bg-green-500" : "bg-zinc-600"}`}
                            />
                            <span
                              className={`text-xs ${trick.certified ? "text-white" : "text-zinc-400"}`}
                            >
                              {trick.name}
                            </span>
                            {trick.certified && (
                              <span className="ml-auto text-xs text-green-500 font-bold">
                                ✓
                              </span>
                            )}
                          </div>
        ...
        <div className="flex flex-wrap gap-4 px-1">
          ...
          <div className="flex items-center gap-2 text-xs text-zinc-400">
            <div
              ...
              style={{ background: branchColor[activeBranch] }}
            />
            Branch
          </div>
          <div className="flex items-center gap-2 text-xs text-zinc-400">
